...
function renderConversations() {
  ...
  for (const conversation of sorted) {
    ...
    item.addEventListener("click", (e) => {
      if (e.target.closest(".conv-archive-btn")) return;
      state.activeConversationId = conversation.id;
      saveState();
      renderConversations();
      renderMessages();
...
function appendMessageElement(message) {
  ...
  if (role === "assistant") {
    ...
    wrapper.innerHTML = `
      ...
      <div class="msg-body">
        <div class="meta">${escapeHtml(agentLabel)}</div>
...
/** 把用户消息推入对话，更新标题并保存渲染 */
function commitUserMessage(conversation, prompt, importedAttachments) {
  conversation.messages.push({
    role: "user",
    content: prompt || "请查看附件",
...
async function sendPrompt(event) {
  ...
  if (!prompt && !attachmentsToSend.length) return;
