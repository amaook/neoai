 ...
 */
async function runSkillAgent({ skillId, task, provider, model, temperature, maxTokens, toolConsent, signal, res }) {
...
export async function streamOpenAICompatible({ provider, model, messages, temperature, maxTokens, enableTools, enabledSkills, toolConsent, signal, res, requestInfo }) {
  requireKey(provider);
  ...
  const baseBody = openAICompatibleChatBody({ provider, model, messages, temperature, maxTokens, stream: true });
  ...
  for (let round = 0; round < (useTools ? maxToolRounds : 1); round++) {
    ...
    for (const tc of toolCalls) {
      ...
      if (tc.name === "invoke_skill") {
        ...
        const result = await runSkillAgent({ skillId: args.skill, task: args.task, provider, model, temperature, maxTokens, toolConsent, signal, res });
        ...
        step = await runToolWithReceipt({ name: tc.name, args, toolConsent, availableToolNames, runner: handleToolCall });
        sseWrite(res, { type: "tool_end", name: tc.name, result: step.result, receipt: step.receipt });
      }
      steps.push(step);
      roundSteps.push(step);
      loopMessages.push({ role: "tool", tool_call_id: tc.id, name: tc.name, content: safeJson(step.result) });
  ...
  for (let round = 0; round < (useTools ? maxToolRounds : 1); round++) {
    ...
    for (const tc of toolCalls) {
      ...
      if (tc.name === "invoke_skill") {
        ...
        step = await runToolWithReceipt({ name: tc.name, args: input, toolConsent, availableToolNames, runner: handleToolCall });
        sseWrite(res, { type: "tool_end", name: tc.name, result: step.result, receipt: step.receipt });
      }
      steps.push(step);
      roundSteps.push(step);
      toolResults.push({ type: "tool_result", tool_use_id: tc.id, content: safeJson(step.result) });
      if (repeatedToolArgumentFailure(steps)) {
        stopHb();
        finishRepeatedToolArgumentFailure(res, steps, requestInfo);
        return;
  ...
  for (let round = 0; round < (useTools ? maxToolRounds : 1); round++) {
    ...
    for (const fc of functionCalls) {
      ...
      if (fc.name === "invoke_skill") {
        ...
        step = await runToolWithReceipt({ name: fc.name, args: fc.args, toolConsent, availableToolNames, runner: handleToolCall });
        sseWrite(res, { type: "tool_end", name: fc.name, result: step.result, receipt: step.receipt });
      }
      steps.push(step);
      roundSteps.push(step);
      // Gemini tool 结果通过 functionResponse 格式返回
      loopMessages.push({ role: "tool", name: fc.name, content: safeJson(step.result) });
      if (repeatedToolArgumentFailure(steps)) {
        stopHb();
        finishRepeatedToolArgumentFailure(res, steps, requestInfo);
        return;
